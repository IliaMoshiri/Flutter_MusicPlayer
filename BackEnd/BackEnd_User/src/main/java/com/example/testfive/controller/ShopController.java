...
public class ShopController {
    ...
    public CommentResponse addComment(@PathVariable String songId,
        ...
        return shopService.addComment(songId, userIdFromHeader(uid), req.getText());
    }
    ...
    public java.util.List<CommentResponse> listComments(@PathVariable String songId){
        return shopService.listComments(songId).stream()
                .map(c -> toDto(c.getId(), c.getUser(), c.getText(), c.getLikes(), c.getDislikes()))
    ...
    public CommentResponse likeComment(@PathVariable Long commentId,
                                       @RequestHeader("X-User-Id") String uid){
        return shopService.voteComment(commentId, userIdFromHeader(uid), CommentVote.VoteType.LIKE);
    }

    @PostMapping("/comments/{commentId}/dislike")
    public CommentResponse dislikeComment(@PathVariable Long commentId,
                                          @RequestHeader("X-User-Id") String uid){
        return shopService.voteComment(commentId, userIdFromHeader(uid), CommentVote.VoteType.DISLIKE);
    }
    ...
    private CommentResponse toDto(Long id, User user, String text, int likes, int dislikes){
    ...
    public CommentResponse addCommentDirect(@RequestBody CommentRequest req,
        ...
        return shopService.addComment(req.getSongId(), userIdFromHeader(uid), req.getText());
    }
